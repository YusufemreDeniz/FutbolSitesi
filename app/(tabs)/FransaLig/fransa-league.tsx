...
export default function FransaLeagueScreen() {
    ...
    const fetchStandings = async () => {
      try {
        const response = await fetch(
          'https://api-football-v1.p.rapidapi.com/v3/standings?league=61&season=2024',
          {
            method: 'GET',
            headers: {
  ...
  return (
    <LinearGradient colors={["#f8fafc", "#e0ecff", "#fff", "#f1f5ff"]} style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        ...
        <Text style={styles.header}>Fransa Ligue 1 Puan Durumu</Text>
        ...
        {loading ? (
          ...
          <View style={{ margin: 16 }}>
            {standings.map((team, idx) => (
              <TouchableOpacity
                key={team.team.id}
                onPress={() => router.push({ pathname: '/(tabs)/FransaLig/team-details', params: { teamId: team.team.id } })}
                activeOpacity={0.8}
                style={[
                  styles.tableRow,
